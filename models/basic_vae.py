...
from .basic_var import FFN, AdaLNEncoderLayer, AdaLNDecoderLayer, AdaLNBeforeHead
...
class Encoder(nn.Module):
    def __init__(
        ...
        if transformer_config is not None:
            self.decoder_layer = AdaLNDecoderLayer(**transformer_config)
            self.dummy_token = nn.Parameter(torch.randn(1, 1, z_channels))
            self.dummy_pos_embedding = 
    def forward(self, x):
        # downsampling
        h = self.conv_in(x)
        for i_level in range(self.num_resolutions):
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.decoder_layer = AdaLNDecoderLayer(**transformer_config) if transformer_config is not None else None
    def forward(self, z):
        # z to block_in
        # middle
